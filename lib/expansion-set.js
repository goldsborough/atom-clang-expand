...
class Entry {
  constructor(marker, originalText) {
    this.marker = marker;
    this.range = marker.getBufferRange();
    this.originalText = originalText;
    console.log('$$', this.range);
  }

  hasSameRowAs(other) {
    return this.range.start == other.range.start.row;
  }
...
export default class ExpansionSet {
  ...
  add(marker, originalText) {
    ...
    const index = findWith(lowerBound, this[expansions], entry.range.start);
    ...
    if (bucket && bucket[0].hasSameRowAs(entry)) {
      bucket.push(entry);
    } else {
  ...
  clear(editor) {
    while (this[expansions].length > 0) {
      this[expansions].pop().unexpand(editor);
    }
  }
