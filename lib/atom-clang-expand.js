...
export default {
  ...
  goTo(locationTriple, message) {
    ...
    atom.workspace.open(locationTriple.file, {
      initialLine: locationTriple.line - 1,
      initialColumn: locationTriple.column - 1,
    }).then(editor => {
  ...
  /**
   * Given the range returned by `insertText`, limits the range to only the
   * text, basically stripping/trimming the range.
   */
  ...
  highlightRange(range) {
    ...
    const marker = editor.markBufferRange(textRange, {invalidate: 'overlap'});
    editor.decorateMarker(marker, {
      type: 'line',
      onlyNonEmpty: true,
  ...
  selectCallRange(editor, call) {
    // Start the range at the start of the line, for formatting.
    const range = new Range(
      new Point(call.begin.line - 1, 0),
  ...
  expand() {
    ...
    this.clangExpand({call: true, rewrite: true}, result => {
      ...
      editor.transact(0, () => {
        ...
        const range = this.indentLines();
        const marker = this.highlightRange(range);
        this.getActiveExpansions().add(marker, originalText);
      });
