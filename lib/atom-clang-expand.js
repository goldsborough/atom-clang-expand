...
export default {
  ...
  deactivate() {
    ...
    this.expansions.clear(atom.project.getActiveTextEditor());
  ...
  expand() {
    ...
    this.clangExpand({call: true, rewrite: true}, result => {
      ...
      editor.transact(0, () => {
        const range = editor.insertText(result.definition.rewritten, {select: true});
        editor.selectToBeginningOfLine();
        editor.selectToEndOfLine();
        editor.autoIndentSelectedRows();
        const marker = this.highlightRange(range[0]);
        this.expansions.add(marker, originalText);
      });
  ...
  unexpand() {
    const editor = atom.workspace.getActiveTextEditor();
    const cursor = editor.getCursorBufferPosition();
    const expansion = this.expansions.remove(cursor);
    if (!expansion) {
